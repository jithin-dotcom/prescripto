...
//       start: dateTime,
//       end: new Date(dateTime.getTime() + 30 * 60 * 1000), // Assume 30-minute appointments
//       resource: app,
//     };
//   });
...
//         <span className="font-medium">{event.resource.user.name}</span>
...
//             <button
//               className="px-2 py-1 rounded text-xs font-medium flex items-center gap-1 bg-purple-500 text-white hover:bg-purple-600 transition-colors"
//               onClick={(e) => {
//                 e.stopPropagation();
//                 navigate("/my-video", {
//                   state: {
...
//           <div className="hidden lg:block bg-white rounded-lg border border-gray-200 overflow-hidden mb-6 shadow-2xl">
//             <div className="p-4">
//               <Calendar
//                 localizer={localizer}
//                 events={events}
...
//         setAppointments(res.data.data || []);
//         setTotalPages(res.data.totalPages || 1);
//         console.log("Appointments:", res.data.data); // Debug log
//       } catch (err) {
//         console.error("Error fetching appointments:", err);
//         toast.error("Failed to fetch appointments");
//       }
...
//   // Prepare events for react-big-calendar
//   const events = appointments.map((app) => {
//     // Parse date (DD/MM/YYYY) and time (h:mm A) using moment
...
//       title: `${app.user.name} - ${app.status}`,
//       start: dateTime,
//       end: moment(dateTime).add(30, "minutes").toDate(), // 30-minute appointment duration
//       resource: app,
//     };
//   });
...
//     newStatus: "confirmed" | "cancelled" | null;
//   }>({ appointmentId: "", newStatus: null });
//   const [view, setView] = useState(Views.WEEK); // Default to Week view for better visibility
//   const navigate = useNavigate();
//   const [pageSize, setPageSize] = useState<number>(5);
...
//             disabled={disabled}
//           >
//             <Ban className="w-2 h-2" /> Cancel
//           </button>
//           {event.resource.status === "confirmed" && event.resource.payment === "paid" ? (
//             <button
//               className="px-1 py-0.5 rounded text-[10px] font-medium flex items-center gap-0.5 bg-purple-500 text-white hover:bg-purple-600 transition-colors"
//               onClick={(e) => {
//                 e.stopPropagation();
//                 navigate("/my-video", {
//                   state: {
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col overflow-x-hidden mt-12">
      ...
      <div className="flex flex-1">
        ...
        <main className="flex-1 w-full min-w-0 p-3 sm:p-4 md:p-6 lg:p-8 pt-20 sm:pt-24">
          ...
          {/* Desktop Calendar View */}
          <div className="hidden lg:block bg-white rounded-lg border border-gray-200 mb-6 shadow-2xl">
            <div className="p-4">
              <Calendar
                localizer={localizer}
                events={events}
