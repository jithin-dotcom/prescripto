...
const CreatePrescription: React.FC = () => {
  ...
  const [appointment, setAppointment] = useState<IAppointmentWithUserResponse | null>(null);

  const myAppointment = state?.appointment;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (diagnosis.trim().length > 30 || diagnosis.trim().length < 3) {
      toast.error("Diagnosis must be below 30 letters and above 3 letters");
      return;
    }

    if (notes.trim().length > 200 || notes.trim().length < 5) {
      toast.error("Notes must be below 200 letters and above 5 letters");
      return;
    }
    ...
    for (let i = 0; i < medicines.length; i++) {
      ...
      for (const field of fields) {
        ...
        if (med[field].trim().length > 50 || med[field].trim().length < 3) {
          toast.error(`Medicine ${i + 1} field "${field}" minimum length should be 3 and maximum length should be 50`);
          return;
        }
      }
    }
