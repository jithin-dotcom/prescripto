...
//             return;
//          }
//         const res = await axiosInstance.get(`/user/user-profile/${id}`);
...
//             </div>
//             {/* <div>
//               <p className="text-sm text-[#5C5C5C]">Password</p>
...
//               <p className="font-medium">{user.houseName}</p>
//             </div>
//             <div>
...
//       try {
//         const res = await axiosInstance.get(`/user/user-profile/${authUser._id}`);
...
//   if (!authUser || !profile) return <div className="p-8 text-gray-600">Loading profile...</div>;
  ...
  useEffect(() => {
    ...
    async function fetchUserProfile() {
      try {
        ...
        // Always call setProfile with fallback/default values
        setProfile({
          name: user?.name || "",
          email: user?.email || "",
          profilePhoto: user?.photo || "",
