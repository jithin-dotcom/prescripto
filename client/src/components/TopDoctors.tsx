...
//         const res = await axiosInstance.get("/doctor/top-doctor-rating")
              ...
              <div className="p-4">
                <div
                  className={`flex items-center gap-2 text-sm ${
                    doctor.doctorId.isVerified ? "text-green-500" : "text-red-500"
                  }`}
                >
                  <span
                    className={`w-2 h-2 rounded-full ${
                      doctor.doctorId.isVerified ? "bg-green-500" : "bg-red-500"
                    }`}
                  ></span>
                  <p>{doctor.doctorId.isVerified ? "Available" : "Unavailable"}</p>
                </div>
                <p className="text-lg font-medium text-[#262626] mt-1">
                ...
                {doctor.averageRating !== undefined && doctor.ratingCount !== undefined && (
                  <div className="flex items-center gap-2 mt-1 text-sm text-yellow-600">
                    <span className="font-semibold">
                      {doctor.averageRating.toFixed(1)} {"⭐".repeat(Math.round(doctor.averageRating))}
                    </span>
